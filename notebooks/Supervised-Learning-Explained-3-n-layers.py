...
def exponential_decay(epoch):
    return np.power(epoch, 0.99)

def sqrt_decay(epoch):
    k = 2
    return k / np.sqrt(epoch + 1)

def log_decay(epoch):
    k = 2
    return k / np.log(epoch + 2)


def stage_decay(epoch):
    if epoch < 100:
        return 1
    elif epoch < 200:
        return 0.9
...
def neural_network_n_layers(X, Y, test_set_x, test_set_y, hidden_layer_dims, activations,
                            ...
                            msg_interval=1, print_interval=100, decay=stage_decay):
    """
    X: numpy                  - shape [n_x, m]
    Y: numpy                  - shape [n_y, m]
...
#     n_y, _ = Y.shape
    ...
    for i in range(num_iterations):
        ...
        # if i > 100 and cost / previous_cost > 1.01:
        #     learning_rate *= 0.6
        previous_cost = cost
...
# param = {"dims": [100], "activations": ["relu", "sigmoid"], "num_iterations": 2001, "learning_rate": 0.01, "stop_cost": 0.05}
param = {"dims": [100, 100], "activations": ["tanh", "tanh", "sigmoid"], "num_iterations": 2001, "learning_rate": 0.01, "stop_cost": 0.05}
W, b, msgs, train_predict, train_accuracy, test_predict, test_accuracy = train_n_layers(param)
